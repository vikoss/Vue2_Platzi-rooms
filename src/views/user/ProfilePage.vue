          ...
          <aside class="profile__aside px-3">
            <div class="profile__card">
              <div class="profile__thumbnail mb-2">
                <img class="profile__image w-full" :src="authUser.avatar">
              </div>
              <div class="text-lg">
                <span class="font-semibold">{{ userRoomsCount }}</span>
                rooms
              </div>
            </div>
          </aside>
...
export default {
  ...
  computed: {
    ...mapGetters(['authUser']),
    userRoomsCount() {
      return this.$store.getters.userRoomsCount(this.authUser['.key']);
    },
  },
};
</script>
